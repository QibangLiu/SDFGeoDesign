...
**Figure 2:On-demand one-to-many unit cell design. Two targets of stress-strain
    ...
    MPa and $\sigma_{0}=40$ MPa, but stress only follows the Ramberg-Osgood equation
    for $\varepsilon \leq 0.156$. For $\varepsilon \in [0.156,0.2]$, the stress increases
    linearly with Young's modulus of $500$ MPa. These two target curves are fed
    into the inverse diffusion model for 500 SDF solutions with guidance weight
    $w=1$, which are then fed into the forward models for prediction. We select
    4 design results with the lowest $L_{2}$ relative error, shown in (c), (d), (e),
