...
class ResidualBlockConvTimeStep(nn.Module):
  def __init__(self, channel_in, channel_out, time_channels, label_emb_dim, groups=None, activation_fn=nn.SiLU(), dropout=None):
    super().__init__()
    lay_list1 = [norm_layer(channel_in, groups), activation_fn]
    if dropout is not None:
        lay_list1.append(nn.Dropout(dropout))
    lay_list1.append(nn.Conv2d(channel_in, channel_out,
                               kernel_size=3, padding="same"))
    self.conv1 = nn.Sequential(*lay_list1)

    self.time_emb = nn.Sequential(
        activation_fn,
        nn.Linear(time_channels, channel_out)
    )

    self.label_emb = nn.Sequential(
        activation_fn,
        nn.Linear(label_emb_dim, channel_out)
    )

    lay_list2 = [norm_layer(channel_out, groups), activation_fn]
    if dropout is not None:
        lay_list2.append(nn.Dropout(dropout))
    lay_list2.append(nn.Conv2d(channel_out, channel_out,
                               kernel_size=3, padding="same"))
...
# %%
# Multi Attention block with shortcut
...
class UNetTimeStep(nn.Module):
  ...
  def __init__(self, img_shape, label_dim, one_hot=False, first_conv_channels=64, channel_mutipliers=[1, 2, 4, 8], has_attention=[False, False, True, True], num_heads=4, num_res_blocks=1, norm_groups=None,
               interpolation="nearest", activation_fn=nn.SiLU(),  dropout=None):
      super().__init__()
      channel_in, img_size = img_shape[0], img_shape[1]
      channel_list = [first_conv_channels *
                      channel_mutiplier for channel_mutiplier in channel_mutipliers]
      ...
      for i in range(len(channel_list)):
        ...
        self.encoder.append(ResidualBlockConvTimeStep(
            in_channel, channel_list[i], time_emb_dim, label_emb_dim, groups=norm_groups, activation_fn=activation_fn, dropout=dropout))
        if has_attention[i]:
          self.encoder.append(MultiAttentionBlock(
        ...
        for _ in range(1, num_res_blocks):
            self.encoder.append(ResidualBlockConvTimeStep(
                channel_list[i], channel_list[i], time_emb_dim, label_emb_dim, groups=norm_groups, activation_fn=activation_fn, dropout=dropout))
            if has_attention[i]:
                self.encoder.append(MultiAttentionBlock(
                    channel_list[i], num_heads=num_heads, groups=norm_groups))
            self.skip_channels.append(channel_list[i])
      ...
      self.bottleneck.append(ResidualBlockConvTimeStep(
          channel_list[-1], channel_list[-1], time_emb_dim, label_emb_dim, groups=norm_groups, activation_fn=activation_fn, dropout=dropout))
      self.bottleneck.append(MultiAttentionBlock(
      ...
      for i in reversed(range(len(channel_list))):
        ...
        if has_attention[i]:
            self.decoder.append(MultiAttentionBlock(
                channel_list[i], num_heads=num_heads, groups=norm_groups))
        for _ in range(1, num_res_blocks):
  ...
  def forward(self, x, timesteps, c, mask):
      ...
      t_emb = self.time_emb(timestep_embedding(
          timesteps, embed_dim=self.first_conv_channels))
...
# %%
model = UNetTimeStep(img_shape=(1, 128, 128),
                     label_dim=51, first_conv_channels=32)
images = torch.randn(2, 1, 128, 128)
timesteps = 500
t = torch.randint(0, timesteps, (len(images),), device='cpu').long()
c = torch.randn(2, 51)
mask = torch.randn(2)
a = model(images, t, c, mask)
...
def beta_scheduler(tot_timesteps, type='linear', s=0.008):
    if type == 'linear':
        ...
        betas = torch.sigmoid(betas)/(betas.max()-betas.min()
                                      )*(0.02-betas.min())/10
        return betas
    elif type == 'cosine':
        """
        ...
        alphas_cumprod = torch.cos(
            ((x / tot_timesteps) + s) / (1 + s) * math.pi * 0.5) ** 2
...
class GaussianDiffusion:
    ...
    #
    def p_mean_variance(self, model, x_t, t, c, w, clip_denoised=True, conditioning=True):
        """
    ...
    def p_sample(self, model, x_t, t, c, w, clip_denoised=True, conditioning=True):
        ...
        model_mean, _, model_log_variance = self.p_mean_variance(model, x_t, t,
                                                                 c, w, clip_denoised=clip_denoised, conditioning=conditioning)
    ...
    @torch.no_grad()
    def p_sample_loop(self, model, image_shape, labels, w=2, clip_denoised=True, conditioning=True):
        ...
        for i in tqdm(reversed(range(0, timesteps)), desc='sampling loop time step', total=timesteps):
            img = self.p_sample(model, img, torch.full(
                (batch_size,), i, device=device, dtype=torch.long), labels, w, clip_denoised, conditioning=conditioning)
    ...
    # sample new images
    @torch.no_grad()
    def sample(self, model, image_shape, labels, w=2, clip_denoised=True, conditioning=True):
        return self.p_sample_loop(model, image_shape, labels, w, clip_denoised, conditioning)
    ...
    def ddim_sample(
        ...
        sample_img = torch.randn(
            (batch_size, *image_shape), device=device)
        ...
        for i in tqdm(reversed(range(0, ddim_timesteps)), desc='sampling loop time step', total=ddim_timesteps):
            ...
            prev_t = torch.full(
                (batch_size,), ddim_timestep_prev_seq[i], device=device, dtype=torch.long)
            ...
            if conditioning:
                pred_noise_c = model(sample_img, t, labels,
                                     torch.ones(batch_size).int().cuda())
                pred_noise = (1+w)*pred_noise_c - w*pred_noise_none
            else:
                pred_noise = pred_noise_none
...
# x_start = torch.randn(28, 28)
# x_noisy = gaussian_diffusion.q_sample(x_start, t=torch.tensor([400]))
...
model = UNetTimeStep(
    ...
    label_dim=10,
    one_hot=True,
    first_conv_channels=96,
    channel_mutipliers=[1, 2, 2],
...
for epoch in range(epochs):
    for step, (images, labels) in enumerate(train_loader):
        ...
        loss = gaussian_diffusion.train_losses(
            model, images, t, labels, batch_mask)

        if step % 100 == 0:
            time_end = time.time()
            print("Epoch{}/{}\t  Step{}/{}\t Loss {:.4f}\t Time {:.2f}".format(epoch +
                  1, epochs, step+1, len_data, loss.item(), time_end-time_start))
...
# %%
labels = torch.ones(64).long().to(device)*int(5)
img_shape = (1, 28, 28)
generated_images = gaussian_diffusion.sample(
    model, img_shape, labels, w=2, clip_denoised=False, conditioning=False)
