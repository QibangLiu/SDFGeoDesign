...
class Decoder(nn.Module):
    ...
    def forward(self, xyt, sdf):
        ...
        # (B,N,Nt,ndim)->(B,N,Nt,ndim*31)
        xyt = encode_position('nerf', position=xyt)
        # (B,N,Nt,ndim*31)->(B,N,Nt,embed_dim)
        x = self.Q_encoder(xyt)
        # (B,N,Nt,embed_dim)->(B,N*Nt,embed_dim)
        x=x.view(B,N*Nt,-1)
...
def TrainNOTModel(infss, filebase, train_flag, epochs=300, lr=1e-3,window_size=None):
    ...
    train_dataloader, test_dataloader, sdf_inverse, su_inverse=LoadDataSU(bs_train=16,bs_test=32)
