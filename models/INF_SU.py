...
import os
if __package__:
    from .configs import InfSU_configs, LoadDataInfSU
...
class ImplicitNeuralFunction(nn.Module):
    def __init__(self,encoder, latent_d=120, in_c=120,
                 ndim=1,out_d=1,
                 d_hidden_sdfnn=[256, 128], emd_version="nerf",
                 padding_value=None):
    ...
    def forward(self, x, sdf):
        """
        Args:
            x (torch.Tensor): [B, N, ndim], query points,
...
def INFModelDefinition(img_shape=(1, 120, 120),
                           ...
                           latent_d=120, in_c=120, ndim=1,out_d=1,
    ...
    INFSSModel = ImplicitNeuralFunction(encoder=EncoderModel,
        latent_d=latent_d, in_c=in_c, ndim=ndim,out_d=out_d, d_hidden_sdfnn=d_hidden_sdfnn, emd_version=emd_version,
...
def EvaluateGeoEncoderModel(trainer,train_loader, test_loader):
...
def TrainGeoEncoderModel(infss, filebase, train_flag, epochs=300, lr=1e-3):
    ...
    train_dataloader, test_dataloader, sdf_inverse, su_inverse=LoadDataInfSU(bs_train=64,bs_test=256)
    ...
    trainer.load_weights(device=device)
    trainer.save_logs()
    EvaluateGeoEncoderModel(trainer,train_dataloader, test_dataloader)
    return trainer


def LoadGeoINFModel(file_base, model_args):
    geo_encoder, sdf_NN = INFModelDefinition(**model_args)
...
if __name__ == "__main__":
    ...
    print(vars(args))
    configs = InfSU_configs()
    filebase = configs["filebase"]
    model_args = configs["model_args"]
    print(f"\n\INFSU Filebase: {filebase}, model_args:")
    print(model_args)
    inf_model = INFModelDefinition(**model_args)
    trainer = TrainGeoEncoderModel(inf_model, filebase, args.train_flag,
                                   epochs=args.epochs, lr=args.learning_rate)
    print(filebase, " training finished")

# %%
# infss = INFModelDefinition(**model_args)
# infss=infss.to(device)
# xyt=torch.randn(2,2000,26,3).to(device)
