...
for i in range(len(out_loop)-1):
    mySketch.Line(point1=vertices[out_loop[i],:2], point2=vertices[out_loop[i+1],:2])

for loop in inner_loops:
    for i in range(len(loop)-1):
        mySketch.Line(point1=vertices[loop[i],:2], point2=vertices[loop[i+1],:2])

# %%
# Create a unit cell part from the sketch
UCPart = myModel.Part(name=UCpart_name, dimensionality=TWO_D_PLANAR, type=DEFORMABLE_BODY)
...
myModel.Part(dimensionality=TWO_D_PLANAR, name=topline_part_name, type= ANALYTIC_RIGID_SURFACE)
myModel.parts[topline_part_name].AnalyticRigidSurf2DPlanar(sketch=mySketch)
del myModel.sketches[sketch_name]
myModel.parts[topline_part_name].ReferencePoint(point=(x_min+width*0.5, y_max, 0))

mySketch = myModel.ConstrainedSketch(name=sketch_name, sheetSize=2*width)
mySketch.Line(point1=(x_min-width*0.5, y_min), point2=(x_max+width*0.5, y_min))
myModel.Part(dimensionality=TWO_D_PLANAR, name=bottomline_part_name, type= ANALYTIC_RIGID_SURFACE)
myModel.parts[bottomline_part_name].AnalyticRigidSurf2DPlanar(sketch=mySketch)
del myModel.sketches[sketch_name]
myModel.parts[bottomline_part_name].ReferencePoint(point=(x_min+width*0.5, y_min, 0))
# %% properties
# Create a material
...
UC_faces_Set = UCPart.Set(faces=UCPart.faces[:], name=all_faces_set_name)  # Apply to all faces of the part
...
myModel.rootAssembly.Instance(name=uc_assem_name, part=myModel.parts[UCpart_name], dependent=ON)
myModel.rootAssembly.Instance(name=topline_assem_name, part=myModel.parts[topline_part_name], dependent=ON)
myModel.rootAssembly.Instance(name=bottomline_assem_name, part=myModel.parts[bottomline_part_name], dependent=ON)
...
if solver == 'explicit':
    ...
    myModel.StaticStep(name=step_name, previous="Initial", timePeriod=1.0, nlgeom=ON,initialInc=0.01, minInc=1e-5, maxInc=0.1, maxNumInc=1000, description="Plasticity analysis step")
else:
...
myModel.fieldOutputRequests['F-Output-1'].setValues(timePoint=
    'TimePoints-2', variables=('S', 'PE', 'PEEQ', 'U','RF','COORD'))

# establish displacement and reaction force recording
del myModel.historyOutputRequests['H-Output-1']
myModel.HistoryOutputRequest(createStepName=step_name, name=
    'H-Output-0', rebar=EXCLUDE, region=
    root_assem.sets['TopEdgeSet'], sectionPoints=DEFAULT,
    variables=('U2', 'RF2'), timePoint= 'TimePoints-2')

if solver == 'explicit':
    myModel.HistoryOutputRequest(createStepName=step_name, name=
        'H-Output-1', timePoint= 'TimePoints-2',
...
amp_name="Amp-1"
myModel.SmoothStepAmplitude(data=((0.0, 0.0), (1.0, 1.0)), name= amp_name, timeSpan=STEP)
myModel.DisplacementBC(name="bot_uy", createStepName="Initial", region=root_assem.sets["BottomEdgeSet"],u1=UNSET, u2=SET, ur3=UNSET)
myModel.DisplacementBC(amplitude=amp_name, name="top_uy", createStepName=step_name, region=root_assem.sets["TopEdgeSet"],u1=UNSET, u2=strain,ur3=UNSET)

# arbitrary node for rm RBM
root_assem.Set(name='anchor_node', nodes=allNodes[:1])
myModel.DisplacementBC(name="anchor_ux", createStepName="Initial", region=root_assem.sets['anchor_node'],u1=SET, u2=UNSET,ur3=UNSET)
...
for idx, node in enumerate(allNodes):
    if np.isclose(node.coordinates[0], 0.):# and (not np.isclose(node.coordinates[1], 0.)) and (not np.isclose(node.coordinates[1], 1.))):
        ...
        root_assem.Set(name='BC_lr_' + str(idx) + 'B', nodes=cur_node_lr_partner)
        # apply periodic boundary conditions in x direction (but both for x and y displacements)
        myModel.Equation(name='pbc-' + str(idx) + '_lr_x', terms=((1.0, 'BC_lr_' + str(idx) + 'A',1), (-1.0, 'BC_lr_' + str(idx) + 'B', 1)))
        myModel.Equation(name='pbc-' + str(idx) + '_lr_y', terms=((1.0, 'BC_lr_' + str(idx) + 'A', 2), (-1.0, 'BC_lr_' + str(idx) + 'B', 2)))
        ...
        root_assem.Set(name='BC_ud_' + str(idx) + 'B', nodes=cur_node_ud_partner)
        myModel.Equation(name='pbc-' + str(idx) + '_ud_x', terms=((1.0, 'BC_ud_' + str(idx) + 'A', 1), (-1.0, 'BC_ud_' + str(idx) + 'B', 1)))
...
if solver=='explicit':
    ...
    myModel.SelfContactStd(createStepName=step_name, interactionProperty=int_cont_prop_name, name=self_cont_name, surface=uc_assem_inst.surfaces[uc_surf_name],thickness=ON)
else:
    raise ValueError("Solver not supported")
...
# %%
#os.system('{} cae -noGUI {}'.format('abaqus2022', os.path.abspath(sys.argv[0])))
