...
class QKVMultiheadCrossAttention(nn.Module):
    def __init__(
        self, heads: int, n_ctx: int, n_data: int
    ):
        super().__init__()
        self.heads = heads
        self.n_ctx = n_ctx
...
class MultiheadCrossAttention(nn.Module):
    def __init__(
        ...
        self.attention = QKVMultiheadCrossAttention(heads=heads, n_ctx=n_ctx, n_data=n_data
                                                    )
...
pc = torch.tensor(
    points_cloud_all[0:2, :, :2], dtype=torch.float32).permute(0, 2, 1).to(device)
