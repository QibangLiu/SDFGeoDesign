...
geo_encoder = EncoderGeo(img_shape, channel_list,
                          has_attention, num_out=output_dim_encoder, first_conv_channels=8, num_res_blocks=1, norm_groups=8)

total_params = sum(p.numel() for p in geo_encoder.parameters())
trainable_params = sum(p.numel() for p in geo_encoder.parameters() if p.requires_grad)
...
filebase = "./saved_model/geo_encoder_unet"
