...
class implicit_sdf(nn.Module):
    def __init__(self, input_size, hidden_sizes, nn_sizes ):
        ...
        for hidden_size in hidden_sizes:
            ...
            self.layers.append((weight, nn.SiLU()))
            in_size = hidden_size
        # the total number of parameters must match the output size of the encoder
        self.num_para = sum([w[0] * w[1] for w, _ in self.layers])

        self.nn_layers = nn.ModuleList()
        for hs in nn_sizes:
            self.nn_layers.append(nn.Linear(in_size, hs))
            self.nn_layers.append(nn.SiLU())
            in_size = hs
        self.nn_layers.append(nn.Linear(in_size, 1))

    def forward(self, x, params_all):
        # x: np,2
        # params_all: nb, number of parameters of output of encoder
        s = 0
        x = x.expand(params_all.shape[0], -1, -1)
        for w_size, af in self.layers:
...
class EncoderGNNModel(torch.nn.Module):
    def __init__(self, num_node_features, graph_hid_dims,fcn_hid_dims, output_dim):
        ...
        for i in range(1,len(graph_hid_dims)):
            ...
            self.layers.append(torch.nn.ReLU())
            self.layers.append(GATConv(in_channels=graph_hid_dims[i], out_channels=graph_hid_dims[i], heads=4, concat=False, dropout=0.2))
            BatchNorm(graph_hid_dims[i])
            self.layers.append(torch.nn.ReLU())
...
sdf_NN = implicit_sdf(input_size=2, hidden_sizes=[16]*4+[14], nn_sizes=[100]*2)
...
geo_encoder = EncoderGNNModel(num_node_features=2, graph_hid_dims=[128]*4,fcn_hid_dims=[100]*4, output_dim=output_dim_encoder)  # Assuming output is a scalar
